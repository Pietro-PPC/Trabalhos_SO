...
int mqueue_create (mqueue_t *queue, int max, int size){
    ...
    queue->sem_item.count = 0;
    queue->sem_vaga.count = max;
    queue->sem_q.count = 1;

    return 0;
}

// Função auxiliar, retorna ponteiro para início da fila
void *queue_pos(mqueue_t *queue, int pos){
    return queue->q + (pos * queue->msg_size);
}

// Função auxiliar, retorna próximo índice circular de ind
int next_ind(mqueue_t *queue, int ind){
    return (ind+1) % queue->max_size;
}
...
int mqueue_send (mqueue_t *queue, void *msg) {
    ...
    if (sem_down(&(queue->sem_vaga))) return -1;
    if (sem_down(&(queue->sem_q))) return -1;

    memcpy(queue_pos(queue, queue->front), msg, queue->msg_size);
    queue->front = next_ind(queue, queue->front);
    queue->num_msgs++;
...
int mqueue_recv (mqueue_t *queue, void *msg) {
    ...
    if (sem_down(&(queue->sem_item))) return -1;
    if (sem_down(&(queue->sem_q))) return -1;

    memcpy(msg, queue_pos(queue, queue->back), queue->msg_size);
    queue->back = next_ind(queue, queue->back);
    queue->num_msgs--;
...
int mqueue_destroy (mqueue_t *queue) {
    ...
    sem_destroy(&(queue->sem_q));

    if (queue->q) free (queue->q);
